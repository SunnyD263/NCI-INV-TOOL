...
If ($_SERVER["REQUEST_METHOD"] == "POST") 
    ...
    If(isset($_POST['UserID']))
        ...
        $SQL=  "SELECT * FROM NCI_User WHERE ([User] = :User)";
        $params = array('User' => $_POST['UserID']);
        $stmt = $Connection->select($SQL, $params);
        if($stmt['count'] == 0)
            {
            $_SESSION["Error"]="";
            unset($_POST['UserID']);
            echo '<span class="ErrorMsg">Neexistující uživatel.</span>';
            Main();
            }
        else
            {
...
function Main()
{
    ...
    {   
        ...
        echo    "<fieldset>";
        echo    "<legend>Přihlášení: </legend><br>";
        echo    "<label for='UserID' class='label-TradeIN'>UserID: </label><br>";
        echo    "<input type='text' id='UserID' name='UserID'  value='' autofocus><br><br>";
        echo    "</fieldset>";
        echo    "</form>";
